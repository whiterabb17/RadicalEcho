...
namespace RadicalEcho.Lib
{
    public class InternalProxy
    {
        ...
        public async Task<bool> Configure()
        {
            ...
            do { Thread.Sleep(1000); } while (!_Configured);
        ...
        private async Task<bool> Proxify(int p1, int p2, int p3)
        {
            try
            {
                ...
                {
                    ZippedToolsDirectory = Path.Combine(Directory.GetCurrentDirectory(), "Libs/TorZipped"),
                    ExtractedToolsDirectory = Path.Combine(Directory.GetCurrentDirectory(), "Libs/TorExtracted"),
                    PrivoxySettings =
                    {
                        Port = p1
                    },
